...
public class JElementParser {
	...
	private final JVal decodeNumber() {
		...
		for (; index < array.length; index++) {
			...
			if (currentChar == '-' || currentChar == '+') {
				if (signed) throw new NumberFormatException("Number signed twice!");
				signed = true;
				if (currentChar == '+') startIndex++;
	...
	private static String repair(String xml, String tag) {
		Pattern pattern = Pattern.compile("<" + tag + ">(.*?)</" + tag + ">");
		Matcher m = pattern.matcher(xml);
		while (m.find()) {
			String escaped = m.group().replace("\"", "");
			xml = xml.replaceAll(m.group(), escaped);
		}
		return xml;
	}
