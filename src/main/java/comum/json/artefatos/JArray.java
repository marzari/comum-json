...
public class JArray implements JElement, Iterable<JElement> {
	...
	public JArray add(Object element, boolean distinct) {
		if (distinct) {
			if (!contains(element)) {
				items.add(Json.build(element));
			}
	...
	public Boolean contains(Object obj) {
		for (JElement objeto : this) {
			if (objeto.isObject()) {
				if (objeto.asObject().toString().equals(obj.toString())) {
					return true;
				}
	...
	public void distinctString() {
		HashSet<String> listWithoutDuplicates = new HashSet<>();
		for (JElement item : items) {
			listWithoutDuplicates.add(item.asVal().string());
		}
