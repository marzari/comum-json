...
import static comum.json.artefatos.JType.OBJECT;
...
import comum.json.stream.JElementSerializer;
...
public class JObject implements JElement, Iterable<Entry<String, JElement>> {
	...
	public <T> T getByType(String path, String type) {
		T retorno;
		switch (type) {
		case "string":
			retorno = (T) get(path, String.class);
			break;
		case "decimal":
			retorno = (T) get(path, BigDecimal.class);
			break;
		case "bigInteger":
			retorno = (T) get(path, BigInteger.class);
			break;
		case "integer":
			retorno = (T) get(path, Integer.class);
			break;
		case "boolean":
			retorno = (T) get(path, Boolean.class);
			break;
		case "date":
			retorno = (T) get(path, LocalDate.class);
			break;
		case "dateTime":
			retorno = (T) get(path, LocalDateTime.class);
			break;
	...
	public JObject json(String path) {
		try {
			return new JElementParser().parse(str(path).toCharArray()).asObject();
		} catch (Exception e) {
			return null;
		}
	}
	...
	public JObject setLiteral(String path, Object element) {
		JElement je = Json.build(element);
		return executeLiteral(path, true, (JObject e, String property) -> {
			e.setLocal(property, je);
			return JObject.this;
		}, this);
	}
	...
	private <R> R executeLiteral(String path, boolean create, BiFunction<JObject, String, R> action, R otherwise) {
		if (path == null) {
			return otherwise;
		}
		return action.apply(this, path);
